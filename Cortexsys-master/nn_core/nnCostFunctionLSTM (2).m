function [J, dJdW, dJdB] = nnCostFunctionLSTM(nn, r, newRandGen)
...
for k=nn.N_l-1:-1:1
    switch nn.l.typ{k+1}
        case nn.defs.TYPES.FULLY_CONNECTED
            ...
            dJdW{k,1} = 1/m*(sum(pagemultWrapper(nn.d{k+1}, nn.A{k}.v, nn.defs), 3) + lambda*nn.W{k});
            dJdB{k} = 1/m*sum(sum(cat(3, nn.d{k+1}), 3), 2);
        case nn.defs.TYPES.RECURRENT
            dJdW{k,nn.rnn.i} = 1/m*(sum(pagemultWrapper(nn.d{k+1}, nn.A{k}.v, nn.defs), 3) + lambda*nn.W{k,nn.rnn.i});
        ...
        case nn.defs.TYPES.LSTM
            ...
            dJdW{k, nn.lstm.i} = 1/m*(sum(pagemultWrapper(nn.d{k+1}, nn.A{k}.v, nn.defs), 3) + lambda*nn.W{k, nn.lstm.i});
