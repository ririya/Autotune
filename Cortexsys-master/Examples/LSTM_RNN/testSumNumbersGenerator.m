...
for i=1:N
%      
   Xlong(end+1) = Xlong(end) + Xlong(end-1);
   if abs(Xlong(end)) > 2
       Xlong(end) = -Xlong(end); 
   end

%  Xlong(end+1) = Xlong(end) +1;
%  if (Xlong(end) > 100)
%      Xlong(end) =1;
%  end
end
...
offset = 1;  
[Y, ~] = getSparseMatrix(Xlong, seqLen, offset,vmap);
% end
...
modelName = 'modelSumNumbers22.mat';
% if ~exist(modelName)
...
seqLen = 2;
% seedMatrix = Xlong(1:seqLen);
seedMatrix = [1 2];
outputSeq = seedMatrix;
...
vsize = numel(vmap);
 Xs = full(vec2map(seedMatrix, vmap));
% Xs = full(ascii2onehot(seedText, vmap));
Xs = [zeros(vsize,1) Xs];
Xtmp = zeros(vsize,1,numel(seedMatrix)+1);
Xtmp(:,1,:) = Xs;
Xs = Xtmp;

nn.disableCuda();
nn.A{1} = varObj(Xs, nn.defs);
preallocateMemory(nn, 1, size(seedMatrix,2)+1);

for t=1:size(seedMatrix,2)+5000

    feedforwardLSTM(nn, 1, seqLen+1, false, true);
    [value,cout] = max(nn.A{end}.v(:,1,seqLen+1));
