...
[X, vmap] = getRandomMatrix(Xlong, seqLen, offset);
offset = 1;  
[Y, ~] = getRandomMatrix(Xlong, seqLen, offset);
% end
...
modelName = 'modelRandomMatrix.mat';
if ~exist(modelName)
...
% seedText = 'ROMEO:';
% seedText = 'Rafael Iriya';

seedMatrix = unidrnd(3,3,6);
...
Xs = [zeros(vsize,1) Xs];
Xtmp = zeros(vsize,1,size(seedMatrix,2)+1);
Xtmp(:,1,:) = Xs;
Xs = Xtmp;

nn.disableCuda();
nn.A{1} = varObj(Xs, nn.defs);
preallocateMemory(nn, 1, size(seedMatrix,2)+2);
% Load up the LSTM with the context
for t=2:size(seedMatrix,2)+1
    feedforwardLSTM(nn, 1, t, false, true);
    [~,cout] = max(nn.A{end}.v(:,1,t));
%     outputText = [outputText  vmap(cout)]
